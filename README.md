# Mukul's Portfolio Website

Welcome to my personal portfolio website! I'm Mukul, and this site serves as a showcase for my work and projects.

## Portfolio Sections

### Projects

#### WEB RTC

- **Description**: Experience seamless real-time communication with our WebRTC-powered platform. Enjoy high-quality video streaming, instant messaging, screen sharing, and more. Whether it's remote collaboration, virtual events, or customer support, our solution, integrated with Agora for reliability, brings people together effortlessly. With robust security measures and a customizable interface, connect and collaborate without boundaries.
- **Demo**: [Live Preview](https://web-rtc-mk.netlify.app/)
- **GitHub Repository**: [GitHub Repository](https://github.com/MK1016o/WebRTC)

#### BadhaaiKart Events

- **Description**: Welcome to BadhaaiKart Events, where celebrations meet perfection! Our platform offers a seamless experience for organizing unforgettable events. From weddings to corporate gatherings, we provide end-to-end event planning services. Explore a curated selection of venues, decor, catering, and entertainment options, tailored to your preferences. Our expert coordinators ensure a stress-free journey, handling every detail to create memorable moments. With a focus on personalization and quality, BadhaaiKart Events transforms your vision into a spectacular reality, leaving you free to cherish every moment of your special occasion.
- **Demo**: [Live Preview](https://badhaaikartevents.netlify.app/)
- **GitHub Repository**: [GitHub Repository](https://github.com/MK1016o/BadhaaiKart-Events)

#### Manage

- **Description**: The “Manage” landing page, a Frontend Mentor Challenge, embodies  a responsive, and visually appealing design showcasing key features of the Manage. Its interactive elements adapt smoothly across devices, inviting exploration of the platform functionalities.
- **Demo**: [Live Preview](https://manage-mk.netlify.app/)
- **GitHub Repository**: [GitHub Repository](https://github.com/MK1016o/Manage)

#### Tic Tac Toe Game

- **Description**: Enjoy the classic game of Tic Tac Toe in a modern, interactive way! Our app offers timeless entertainment with a sleek interface and multiplayer functionality. Challenge friends or play against the AI for strategic fun anytime, anywhere. Experience the nostalgic thrill of this beloved game with intuitive controls and engaging visuals. Simple yet addictive, our Tic Tac Toe app promises endless hours of competitive enjoyment for players of all ages.
- **Demo**: [Live Preview](https://tic-tac-toe-mk1016o.netlify.app/)
- **GitHub Repository**: [GitHub Repository](https://github.com/MK1016o/Tic-Tac-Toe)

### Skills

- HTML5
- CSS3
- JavaScript
- Web Development
- Front-End Development
- Responsive Design
- Java
- Python
- C/C++

### Resume

You can view or download my resume [here](https://drive.google.com/file/d/1EBrMCEKWDQc78TYi2Ftdno7pqVedH6aw/view?usp=drive_link


### Contact Information

- Email: [mukulsahu966@gmail.com](mailto:mukulsahu966@gmail.com)

- LinkedIn: [Mukul .](https://www.linkedin.com/in/mk1016o/)

- GitHub: [Mukul](https://github.com/MK1016o)

## Customization

If you'd like to use this template for your own portfolio website:

1. Fork this repository.
2. Customize the content in the HTML and CSS files to reflect your information and style.
3. Deploy the website using a platform like GitHub Pages, Netlify, or Vercel.

## Contributing

Contributions are not typically accepted for personal portfolios, but feel free to adapt this template for your own use.


#### Made with 💖 by MK
